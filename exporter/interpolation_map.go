...
func (i *interpolationMap) Prune(client sarama.Client, maxOffsets int) {
	klog.V(DEBUG).Info("pruning iMap data", "maxOffsets", maxOffsets)
	if i.iMap == nil {
		klog.V(DEBUG).Info("Interpolation map is nil, nothing to prune")
		return
	}
	admin, err := sarama.NewClusterAdminFromClient(client)
	if err != nil {
		klog.Errorf("Error creating cluster admin", "err", err.Error())
	}
	if admin == nil {
		klog.Errorf("Failed to create cluster admin")
		return
	}
	...
	i.mu.Lock()
	klog.V(DEBUG).Info("iMap locked for pruning")
	start := time.Now()
		...
		for topic, partitions := range i.iMap[group] {
			...
			for partition, offsets := range partitions {
				if len(offsets) > maxOffsets {
					...
					klog.V(DEBUG).Info("pruning offsets", "count", len(offsetKeys), "group", group, "topic", topic, "partition", partition)
	...
	klog.V(DEBUG).Info("pruning complete", "duration", time.Since(start).String())
	i.mu.Unlock()
}
